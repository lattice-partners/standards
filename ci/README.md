...
Pin to a tag (`@v0.8.0`), not `@main`, in real projects.

## workflows/ci.yml

A ready CI workflow that runs `standards-check` on push to `main`, `dev`, and
release tags. Copy it to `.github/workflows/ci.yml`, or let `lattice init`
place it for you (it pins the action to the standard version automatically).
